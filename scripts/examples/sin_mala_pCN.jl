...
ρ(U, ∇U, u, v, δ) = U + 0.5*dot(v-u, ∇U) + δ*0.25*(dot(v+u, ∇U) + norm(∇U)^2)

function pcn_mala_sampler(X::AbstractModel, T::Float64, L::Int64,
        ...
        count = 0
        for i in 2:niter
                ξ₁  = ((2-δ)*ξ₀ .- 2δ*∇U₀ .+ sqrt(8*δ)*randn(length(ξ₀)))/(2+δ)
                # @assert ξ₁ == ξ₀
                # @show dt
                # @show N
                ∇U₁, U₁ = get_info(X, S, ξ₁, u, v, dt, N)
                ...
                acc_rej =  ρ(U₀, ∇U₀, ξ₀,  ξ₁, δ) - ρ(U₁, ∇U₁, ξ₁, ξ₀, δ)
                # @show acc_rej
                # @assert acc_rej == 0.0
                if acc_rej > log(rand())
                        ∇U₀, U₀, ξ₀ = ∇U₁, U₁, ξ₁
                ...
                if i % 100 == 0
                    ...
                    #println("Adaptive step: ar: $(count/100), new tau: $δ")
                    count = 0
                end
                ξtrace[:,i] = ξ₀
        end
...
function runall_pcn_mala(SHORT = false, hist = false)
    Random.seed!(0)
    T = 50.0
    niter = 140000
    L = 6
    ...
    v = 3π
    X = SinSDE(α, L, T)
    time = @elapsed (XX = pcn_mala_sampler(X, T, L, u, v, niter))
    # if SHORT == false
    #     if hist
