...
function λbar(n, S::System, X::SinSDE, u, v, t::Float64)
    a = S.ϕ[n].δ*(X.α*X.α + X.α)*0.5*abs(S.θ[n])
    b = S.ξ[n]*S.θ[n]
    c = S.θ[n]*S.θ[n]
    waiting_time(a,b,c)
end
#OLD IMPLEMENTATION WHICH WAS NOT CLEAR TO USER BUT IS WORKING
# function λbar(n, S::System, X::SinSDE, u, v, t::Float64)
#     ran = rand()
...
function runall_zz(SHORT = false, hist = false)
    Random.seed!(0)
    T = 50.0
    clock = 10000.0
    L = 6
    ...
    v = 3*Float64(π)
    X = SinSDE(α, L, T)
    time = @elapsed (XX = zz_sampler(X, T, L, u, v, clock))
    # if SHORT == false
    #     if hist
