...
function boomerang_count_ub(α::Float64, T::Float64, L::Int64, u::Float64, v::Float64, clock::Float64)
    N = 2^(L+1)-1
    num_events = fill(0, N)
    ξ = zeros(N)
    θ = randn(N)
...
function run_all()
    ...
    v = 3*Float64(π)
    X = SinSDE(α, L, T)
    zz_count = zz_sampler_count_ub(X, T, L, u, v, clock, ξ, θ)
    xx_count = boomerang_count_ub(α, T, L, u, v, clock)
...
savefig("./boomerang/events_comparison/comparison_ub1.pdf")
